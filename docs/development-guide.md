...
- 列表字段显隐：默认精简展示名称、IP、网页 URL、状态、操作；可显示资源、投放屏幕、RTSP、ONVIF 等长字段。
- 资源监控：默认折叠为项目汇总摘要，仍保持自动轮询；展开后显示 CPU、内存、网络、磁盘读写详情，单路容器资源可通过列表字段显示。
- 单行启动、停止、重启。
- 配置面板：启动、停止、重启、日志、编辑、复制为新视频源、复制 RTSP、复制 IP、复制 mpv 命令、删除。配置面板只通过再次点击当前行“配置”或点击其他行“配置”切换收起。
- 批量启动、停止、重启。
- 定时状态刷新。
...
- 点击已经绑定到矩阵围栏的视频源，会打开矩阵场景下的编辑窗口，只修改名称和网页 URL。
- 围栏内打开按钮指向视频源的网页 URL，用于核对原始大屏页面。
...
`scripts/seed-demo-cameras.sh` 会通过登录接口获取会话令牌，然后调用现有 API 幂等创建演示项目、大屏地址、默认停止的视频源、`viewer01` 和 `operator01` 演示用户及项目授权。脚本默认使用 `admin / admin123456` 和 `http://127.0.0.1:8177/api`，可通过 `ADMIN_USERNAME`、`ADMIN_PASSWORD`、`DEMO_USER_PASSWORD` 和 `API_BASE` 覆盖。脚本不直接写 SQLite，也不会立即启动视频源容器。
