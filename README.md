...
- `container/`：构建 `virtualwebcam:latest`，单个容器负责把一个网页转为 RTSP，并通过 go2rtc 提供 ONVIF。
- `backend/` + `frontend/`：Web 管理后台，用 Node.js、Express、SQLite、Vue3 管理多路摄像头容器。
...
每一路摄像头对应一个 `virtualwebcam:latest` 容器。容器内部启动：
...
```bash
cd backend
PORT=8177 DOCKER_NETWORK=onvif_macvlan VIRTUALWEBCAM_IMAGE=virtualwebcam:latest npm run dev
...
绑定关系只作为管理元数据保存，不改变 RTSP/ONVIF 输出地址；真实投屏或中控接入仍然使用摄像头的 ONVIF/RTSP 地址。
...
- `GET /api/projects/:id/export`：导出项目配置 JSON，包含项目、摄像头、绑定、RTSP/ONVIF 地址
